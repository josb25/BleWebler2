...
<div class="library" class:dropping ondragenter={onDragEnter} ondragleave={onDragLeave} ondragover={onDragOver} ondrop={onDrop}>
    <header class="hero">
        <div class="hero-copy">
            ...
            <h1>Designs</h1>
            <p>Open any design and print it. Designs with fields let you fill in new values first.</p>
            <div class="readout" aria-label="Catalogue summary">
                <span>{entries.length} designs</span>
                <span>{fillableCount} with fields</span>
                <span class="local">stored on this device</span>
            </div>
        </div>
    ...
    {:else}
        <div class="gallery">
            {#each visible as entry (`${entry.template.id}#${entry.savedAt}`)}
                <article class="card">
                    <button class="thumb-button" onclick={() => open(entry)} title={entry.template.params.length > 0 ? `Fill in “${entry.template.name}”` : `Edit “${entry.template.name}”`}>
                        ...
                        <span class="primary-action">{entry.template.params.length > 0 ? 'Fill in' : 'Edit'}</span>
                    </button>
                    <button class="star" class:active={entry.favorite === true} title={entry.favorite ? 'Remove from favorites' : 'Add to favorites'} aria-label={entry.favorite ? 'Remove from favorites' : 'Add to favorites'} onclick={() => toggleFavorite(entry.template.id)}><Icon name="star" size={16} fill={entry.favorite === true} /></button>
                    <div class="meta">
...
<style>
    ...
    .library.dropping { outline: 2px dashed var(--accent); outline-offset: 8px; border-radius: 12px; }
    .library.dropping::after { content: 'Drop design files to import'; position: fixed; left: 50%; bottom: 28px; transform: translateX(-50%); z-index: 30; padding: 9px 16px; border-radius: 999px; background: var(--accent); color: #fff; font-size: 13px; font-weight: 700; box-shadow: var(--shadow); }
    .hero { display: flex; align-items: flex-end; justify-content: space-between; gap: 20px; flex-wrap: wrap; }
    .hero-copy { min-width: 240px; flex: 1; }
    .eyebrow { margin: 0 0 5px; color: var(--accent); font-family: var(--mono); font-size: 11px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; }
    .hero h1 { margin: 0; font-size: clamp(32px, 5vw, 46px); line-height: 1; letter-spacing: -.045em; }
    .hero-copy > p:not(.eyebrow) { margin: 8px 0 0; max-width: 56ch; color: var(--muted); font-size: 14px; }
    .hero-actions { display: flex; gap: 8px; flex-wrap: wrap; }
    .hero-actions button { min-height: 40px; }
    .readout { display: flex; flex-wrap: wrap; margin-top: 13px; color: var(--muted); font-family: var(--mono); font-size: 10px; letter-spacing: .06em; text-transform: uppercase; }
    .readout span { padding: 0 10px; border-left: 1px solid var(--border); }
    .readout span:first-child { padding-left: 0; border-left: 0; }
    .readout .local { color: var(--ok); }
    .catalogue-bar { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; padding: 10px; border: 1px solid var(--border); border-radius: 12px; background: var(--panel); }
    ...
    .search input { width: 100%; padding: 10px 36px; border-radius: 8px; }
    ...
    .scopes { display: flex; gap: 4px; padding: 3px; border-radius: 8px; background: var(--panel-2); }
    ...
    .card { position: relative; min-width: 0; overflow: hidden; border: 1px solid var(--border); border-radius: var(--card-radius, 7px); background: var(--panel); transition: border-color .12s ease, box-shadow .12s ease, transform .12s ease; }
    ...
    .primary-action { position: absolute; left: 50%; bottom: 9px; transform: translate(-50%, 5px); opacity: 0; padding: 4px 12px; border-radius: 999px; background: var(--accent); color: #fff; font-size: 12px; font-weight: 700; transition: opacity .12s ease, transform .12s ease; }
    .card:hover .primary-action, .thumb-button:focus-visible .primary-action { opacity: 1; transform: translate(-50%, 0); }
    .star { position: absolute; top: 7px; right: 7px; z-index: 2; width: 31px; height: 31px; min-height: 0; padding: 0; border: 0; border-radius: 50%; background: rgb(0 0 0 / 38%); color: #fff; box-shadow: none; backdrop-filter: blur(2px); }
    .star:hover { transform: none; background: rgb(0 0 0 / 55%); }
    ...
    .tags button { min-height: 0; padding: 2px 6px; border-radius: 999px; border-color: var(--border); background: var(--panel-2); box-shadow: none; color: var(--muted); font-size: 10px; }
    ...
    .card-actions { display: flex; align-items: center; gap: 4px; padding-top: 5px; border-top: 1px solid var(--border); }
    .card-actions .spacer { flex: 1; }
    .open-action { min-height: 28px; padding: 4px 9px; border-color: transparent; background: transparent; box-shadow: none; color: var(--accent); font-size: 12px; font-weight: 700; }
    .open-action:hover, .icon:hover { transform: none; background: var(--panel-2); }
    ...
    .empty { min-height: 260px; padding: 36px 18px; border: 1px dashed var(--border); border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 9px; color: var(--muted); text-align: center; }
    ...
    .import-report { padding: 10px 12px; border: 1px solid var(--border); border-radius: 9px; display: flex; align-items: flex-start; gap: 10px; flex-wrap: wrap; background: var(--panel); font-size: 12px; }
